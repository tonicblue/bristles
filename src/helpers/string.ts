...
export default class StringHelpers {
  ...
  static _startsWith(input: string, test: string): boolean|string {
    try {
      const helper: HelperOptions = arguments[arguments.length - 1];
      const startsWith = typeof input === 'string' && typeof test === 'string' && input.startsWith(test);
      if (!helper.fn) {
        return startsWith;
      } else {
        if (startsWith) {
  ...
  static _endsWith(input: string, test: string): boolean|string {
    try {
      const helper: HelperOptions = arguments[arguments.length - 1];
      const startsWith = typeof input === 'string' && typeof test === 'string' && input.endsWith(test);
      if (!helper.fn) {
        return startsWith;
      } else {
        if (startsWith) {
  ...
  static _regexMatch(input: string, expression: string, options?: string): false|string[]|string {
    try {
      ...
      let output: false|string[]|string = matches;
